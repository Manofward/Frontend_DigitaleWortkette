...
1. When you want to leave the lobby as a player or close the lobby as a host a post is made for leaving the lobby
   1. so that the lobby is deleted(host)
      1. when you go from host-lobby page back to homepage through the back or home button the lobby needs to be deleted
   2. so that the player is deleted from the player list (player)
      1. feature for leaving lobby needs to be added that when you leave the lobby as a player your username is deleted from the playerlist of the lobby
